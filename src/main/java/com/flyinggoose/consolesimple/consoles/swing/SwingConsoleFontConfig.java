...
import com.flyinggoose.consolesimple.utils.Symbols;
...
public class SwingConsoleFontConfig {
    ...
    protected SwingConsoleFontConfig(boolean useAntiAliasing, BoldMode boldMode, Font... fontsInOrderOfPriority) {
        if (fontsInOrderOfPriority == null || fontsInOrderOfPriority.length == 0) {
            throw new IllegalArgumentException("Must pass in a valid list of fonts to SwingConsoleFontConfig");
        }
    ...
    private static List<Font> getDefaultWindowsFonts() {
        ...
        return Collections.unmodifiableList(Arrays.asList(
                getAndRegisterFont("fonts/bpdots.squares-bold.otf", fontSize),
    ...
    private static Font getAndRegisterFont(String location, int size) {
        try {
            Font font = Font.createFont(Font.TRUETYPE_FONT, getResource(location));
    ...
    private static File getResource(String relativePath) {
        URL url = SwingConsole.class.getClassLoader().getResource(File.separator + relativePath.replace("/", File.separator).replace("\\", File.separator));
        File file = null;
        try {
     ...
     *
     * @return An {@link SwingConsole} font configuration object with default values set up
     */
    public static SwingConsoleFontConfig getSystemDefault() {
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public static SwingConsoleFontConfig newInstance(Font... fontsInOrderOfPriority) {
        return new SwingConsoleFontConfig(true, BoldMode.EVERYTHING_BUT_SYMBOLS, fontsInOrderOfPriority);
    }
     ...
     * to this {@link SwingConsoleFontConfig} in the order of priority specified and chose the first font which is
